...
use ooxmlsdk::{
    ...
    parts::{
        image_part::ImagePart, notes_master_part::NotesMasterPart,
        notes_slide_part::NotesSlidePart, presentation_document::PresentationDocument,
        presentation_part::PresentationPart, slide_layout_part::SlideLayoutPart,
        slide_master_part::SlideMasterPart, slide_part::SlidePart, theme_part::ThemePart,
    },
    schemas::{
        ...
        schemas_openxmlformats_org_presentationml_2006_main::{
            ...
            NotesMaster, NotesMasterId, NotesMasterIdList, NotesSize, NotesSlide, Presentation,
            Shape, ShapeProperties, ShapeTree, ShapeTreeChoice, Slide, SlideId, SlideIdList,
            SlideLayout, SlideLayoutId, SlideLayoutIdList, SlideMaster, SlideMasterId,
            SlideMasterIdList, SlideSize, TextBody,
        },
    },
...
pub fn convert_ppt_with_options(
    ...
    for (slide_index, source_slide) in slides.into_iter().enumerate() {
        ...
        let layout_part =
            if let Some((master_index, layout_index)) = find_layout(&masters, master_id) {
        ...
        match source_slide.notes {
            PptLiveNotesLink::Resolved { object, .. } => {
                let notes_part =
                    slide_part.add_new_part_auto_id::<_, NotesSlidePart>(&mut document)?;
                ...
                notes_part.set_root_element(
                    ...
                    NotesSlide {
                        ...
                        color_map_override: Some(Box::new(p::ColorMapOverride {
                            color_map_override_choice: Some(
                                p::ColorMapOverrideChoice::MasterColorMapping,
                            ),
                        })),
                        ..Default::default()
                    },
    ...
    presentation_part.set_root_element(
        ...
        Presentation {
            ...
            slide_size: Some(SlideSize {
                cx: i32::try_from(slide_width).map_err(|_| {
                    olecfsdk::Error::Limit("PPT slide width exceeds OOXML i32".into())
                })?,
                cy: i32::try_from(slide_height).map_err(|_| {
                    olecfsdk::Error::Limit("PPT slide height exceeds OOXML i32".into())
                })?,
                ..Default::default()
            }),
...
fn ppt_transition_choice(
    ...
    let choice = match effect_type {
        ...
        26 if matches!(direction, 1 | 2 | 3 | 4 | 8) => {
            Choice::WheelTransition(p::WheelTransition {
...
fn add_layout<'source>(
    ...
    if source.source_master_id.is_none()
        && let Some(index) = master.layouts.iter().position(|layout| {
            layout.source_master_id.is_none() && layout.geometry == source.geometry
        })
    {
        return Ok(index);
    }
                ...
                .map(|(column_index, _)| {
                    ...
                    Ok(a::TableCell {
                        row_span: (is_origin && cell.row_span > 1)
                            ...
                            .transpose()
                            .map_err(|_| {
                                olecfsdk::Error::Limit("PPT table row span exceeds i32".into())
                            })?,
                        grid_span: (is_origin && cell.column_span > 1)
                            .then(|| i32::try_from(cell.column_span))
                            .transpose()
                            .map_err(|_| {
                                olecfsdk::Error::Limit("PPT table column span exceeds i32".into())
                            })?,
                        horizontal_merge: (!is_origin && column_index > cell.column)
                            .then_some(BooleanValue::True),
                        vertical_merge: (!is_origin && row_index > cell.row)
                            .then_some(BooleanValue::True),
                        text_body,
                ...
                graphic_data_choice: vec![a::GraphicDataChoice::Table(Box::new(a::Table {
                    ...
                    table_grid: a::TableGrid {
                        grid_column: source
                            ...
                            .map(|column| {
                                Ok(a::GridColumn {
                                    width: CoordinateValue::Emu(master_to_emu(
    ...
    let color_scheme = a::ColorScheme {
        ...
        followed_hyperlink_color: Box::new(a::FollowedHyperlinkColor {
            followed_hyperlink_color_choice: Some(
                a::FollowedHyperlinkColorChoice::RgbColorModelHex(ppt_theme_rgb(followed)),
            ),
        }),
        ..Default::default()
    };
    ...
    a::FormatScheme {
        ...
        fill_style_list: a::FillStyleList {
            fill_style_list_choice: [colors[4], colors[5], colors[2]]
                .into_iter()
                .map(|color| {
                    a::FillStyleListChoice::SolidFill(Box::new(ppt_theme_solid_fill(color)))
                })
                .collect(),
        },
        ...
        background_fill_style_list: a::BackgroundFillStyleList {
            background_fill_style_list_choice: [colors[0], colors[4], colors[5]]
                .into_iter()
                .map(|color| {
                    a::BackgroundFillStyleListChoice::SolidFill(Box::new(ppt_theme_solid_fill(
                        color,
                    )))
                })
                .collect(),
...
fn convert_placeholder(source: &olecfsdk::ppt::PlaceholderAtom) -> Option<p::PlaceholderShape> {
    let (r#type, orientation) = match source.placement_id {
        ...
        PptPlaceholderType::MasterBody | PptPlaceholderType::Body => {
