use olecfsdk::doc::{
    DocBlockRef, DocCommentRef, DocComments, DocCp, DocCpRange, DocDataNodeValue,
    DocDocumentPartRef, DocFieldRef, DocFile, DocNoteKind, DocNotes, DocOfficeArtColor,
    DocOfficeArtFill, DocOfficeArtImageLink, DocOfficeArtLine, DocOfficeArtShapeRef,
    DocOfficeArtTextInsets, DocParagraphRef, DocSectionProperties, DocShapeAnchorRef,
    DocSpecialContentRef, DocTableCellRef, DocTableRef, DocTables, DocTextPieceValueRef,
    DocTextRangeRef, DocTextboxes, FieldDocumentPart, FieldEndFlags, GrpPrl, KnownSprm,
    PicfAndOfficeArtData, Prl, PrmPropertiesRef, Sprm, SprmGroup, SprmKind, SprmOperand,
    SprmOperandSize, StyleFormatting, StyleGeneralFlags, StyleKind,
};
...
use ooxmlsdk::{
    ...
    schemas::{
        ...
        schemas_openxmlformats_org_wordprocessingml_2006_main::{
            AdjustRightIndent, AutoRedefine, AutoSpaceDe, AutoSpaceDn, BasedOn, BiDi, Body,
            BodyChoice, Bold, BoldComplexScript, BookmarkEnd, BookmarkStart, Caps, Columns,
            Comment, CommentChoice, CommentRangeEnd, CommentRangeStart, CommentReference, Comments,
            ContextualSpacing, Document, DoubleStrike, Drawing, DrawingChoice, Emboss, Endnote,
            EndnoteChoice, EndnoteReference, Endnotes, FieldChar, FieldCharValues, FieldCode,
            FontSize, FontSizeComplexScript, Footnote, FootnoteChoice, FootnoteReference,
            Footnotes, GutterOnRight, Imprint, Indentation, Italic, ItalicComplexScript,
            Justification, JustificationValues, KeepLines, KeepNext, Kinsoku,
            LineSpacingRuleValues, LinkedStyle, Locked, MirrorIndents, NextParagraphStyle, NoProof,
            Outline, OutlineLevel, OverflowPunctuation, PageBreakBefore, PageMargin,
            PageNumberType, PageOrientationValues, PageSize, Paragraph, ParagraphChoice,
            ParagraphProperties, ParagraphStyleId, Personal, PersonalCompose, PersonalReply,
            PrimaryStyle, RightToLeftText, Run, RunChoice, RunProperties, RunPropertiesChoice,
            SectionMarkValues, SectionProperties, SectionType, SemiHidden, Shadow, SmallCaps,
            SnapToGrid, SpacingBetweenLines, Strike, Style, StyleHidden, StyleName,
            StyleParagraphProperties, StyleRunProperties, StyleValues, Styles, SuppressAutoHyphens,
            SuppressLineNumbers, SuppressOverlap, Table, TableCell, TableCellChoice, TableChoice2,
            TableRow, TableRowChoice, Text, TextBoxContent, TextBoxContentChoice, TextType,
...
pub fn convert_doc_with_options(
        ...
        match block {
            DocBlockRef::Paragraph(paragraph) => {
                ...
                if intermediate_sections
                    .get(next_section)
                    .is_some_and(|section| {
                        section.local_cp_range().end == paragraph.local_cp_range().end
                    })
                {
                    converted
            ...
            DocBlockRef::Table(table) => {
                while intermediate_sections
                    .get(next_section)
                    .is_some_and(|section| {
                        section.local_cp_range().end <= table.local_cp_range().end
                    })
                {
    ...
    for image in media.pending {
        let image_part = main_part.add_image_part_with_id(
            &mut document,
            image.content_type,
        ...
        let mapped = match property.sprm.kind() {
            ...
            SprmKind::Known(KnownSprm::SCcolumns) => {
                ...
                target
                    ...
                    .get_or_insert_with(Columns::default)
                    .column_count =
                    Some(i16::try_from(value + 1).expect("section columns fit i16"));
                true
            }
            SprmKind::Known(KnownSprm::SDxaColumns) => {
                let Some(value) = word_u16(&property.operand).filter(|value| *value <= 31_680)
                else {
                    has_unmapped = true;
                    continue;
                };
...
fn collect_note_references(
    ...
    for note in footnotes.notes().iter().chain(endnotes.notes()) {
        ...
        if id > 32_767 {
            return Err(olecfsdk::Error::Limit(
                "DOC note index exceeds OOXML note ID range".into(),
            )
            .into());
        }
        ...
        for note in footnotes.notes() {
            ...
            converted.push(Footnote {
                ...
                .map(|block| match block {
                    ConvertedStoryBlock::Paragraph(paragraph) => {
...
fn convert_comments_part<'a>(
    ...
    let mut fields =
        DocFieldCursor::new(collect_document_field_spans(first.text().document_part()));
...
const fn days_in_month(year_offset: u16, month: u8) -> u8 {
    match month {
        ...
        2 if (1900 + year_offset).is_multiple_of(400)
            || ((1900 + year_offset).is_multiple_of(4)
                && !(1900 + year_offset).is_multiple_of(100)) =>
        {
            29
        }
        2 => 28,
...
fn convert_comment_content<'a>(
    ...
    for block in text.blocks_with_tables(&tables)?.blocks() {
        ...
        result.push(match block {
            DocBlockRef::Paragraph(paragraph) => {
                CommentChoice::Paragraph(Box::new(convert_paragraph(
                    *paragraph,
...
fn convert_style_run_property(target: &mut StyleRunProperties, property: &Prl) -> bool {
    match property.sprm.kind() {
        ...
        SprmKind::Known(KnownSprm::CHps) => {
            let Some(value) =
                word_u16(&property.operand).filter(|value| (2..=3276).contains(value))
            else {
                return false;
            };
    ...
    Ok(Some(RunChoice::Drawing(Box::new(Drawing {
        drawing_choice: Some(DrawingChoice::Anchor(Box::new(wp::Anchor {
            ...
            relative_height: Some(
                u32::try_from(office_shape.z_order()).map_err(|_| {
                    olecfsdk::Error::Limit("DOC OfficeArt z-order exceeds u32".into())
                })?,
            ),
            ...
            vertical_position: Some(Box::new(wp::VerticalPosition {
                ...
                vertical_position_choice: Some(wp::VerticalPositionChoice::PositionOffset(
                    geometry.top,
                )),
            })),
            extent: wp::Extent {
        ...
        graphic_data: a::GraphicData {
            ...
            graphic_data_choice: vec![a::GraphicDataChoice::WordprocessingShape(Box::new(
                wps::WordprocessingShape {
                    non_visual_drawing_properties: Some(Box::new(
                        wps::NonVisualDrawingProperties {
                            id: shape_id,
                    ...
                    wordprocessing_shape_choice1: Some(
                        if matches!(office_shape.shape_type(), 20 | 32..=40) {
                            wps::WordprocessingShapeChoice::NonVisualConnectorProperties(
                                Box::default(),
                            )
                        } else {
                            wps::WordprocessingShapeChoice::NonVisualDrawingShapeProperties(
                                Box::new(wps::NonVisualDrawingShapeProperties {
                    ...
                    shape_properties: Some(Box::new(wps::ShapeProperties {
                        ...
                        shape_properties_choice1: Some(wps::ShapePropertiesChoice::PresetGeometry(
                            Box::new(a::PresetGeometry {
                                preset,
        ...
        graphic_data: a::GraphicData {
            ...
            graphic_data_choice: vec![a::GraphicDataChoice::Picture(Box::new(pic::Picture {
                ...
                shape_properties: Some(Box::new(pic::ShapeProperties {
                    transform2_d: Some(Box::new(floating_transform(office_shape, geometry))),
                    shape_properties_choice1: Some(pic::ShapePropertiesChoice::PresetGeometry(
                        Box::new(a::PresetGeometry {
...
fn floating_transform(
    ...
    a::Transform2D {
        horizontal_flip: (shape_horizontal_flip ^ geometry.reverse_horizontal)
            .then_some(true.into()),
    ...
    Ok(Some(RunChoice::Drawing(Box::new(Drawing {
        drawing_choice: Some(DrawingChoice::Inline(Box::new(wp::Inline {
            ...
            graphic: Box::new(a::Graphic {
                graphic_data: a::GraphicData {
                    uri: "http://schemas.openxmlformats.org/drawingml/2006/picture".into(),
                    graphic_data_choice: vec![a::GraphicDataChoice::Picture(Box::new(
                        pic::Picture {
                            non_visual_picture_properties: Some(Box::new(
                            ...
                            blip_fill: Some(Box::new(pic::BlipFill {
                                ...
                                blip_fill_choice: Some(pic::BlipFillChoice::Stretch(Box::new(
                                    a::Stretch {
                                        fill_rectangle: Some(Default::default()),
                                        ..Default::default()
                                    },
                            ...
                            shape_properties: Some(Box::new(pic::ShapeProperties {
                                ...
                                shape_properties_choice1: Some(
                                    pic::ShapePropertiesChoice::PresetGeometry(Box::new(
                                        a::PresetGeometry {
                                            preset: a::ShapeTypeValues::Rectangle,
                                            adjust_value_list: Some(Default::default()),
                                            ..Default::default()
