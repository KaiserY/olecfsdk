...
impl SmartTagFactoidTypeId {
    ...
    fn from_u32(value: u32) -> Result<Self> {
        match u16::try_from(value) {
            ...
            Err(_) if value == Self::MALFORMED_CVE_2016_3133_VALUE => {
                Ok(Self::MalformedCve20163133)
            }
...
impl Clx {
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>> {
        ...
        for run in &self.property_runs {
            ...
            let length = i16::try_from(grpprl.len())
                .map_err(|_| Error::Limit("Prc grpprl exceeds i16".into()))?;
        ...
        push_u32(
            &mut bytes,
            u32::try_from(piece_table.len())
                .map_err(|_| Error::Limit("PlcPcd exceeds u32".into()))?,
        );
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let mut next_string = |name: &str| -> Result<Vec<u8>> {
            ...
            let length = remaining
                ...
                .ok_or_else(|| {
                    Error::invalid(offset as u64, format!("unterminated PrDrvr {name}"))
                })?;
...
impl OleObjectDescriptor {
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let persist2 = (bytes.len() == 6).then(|| {
            OleObjectPersist2Flags::from_bits_retain(u16::from_le_bytes([bytes[4], bytes[5]]))
        });
        Ok(Self {
            persist1,
            clipboard_format,
...
impl ListLevel {
        ...
        let (paragraph_properties, paragraph_incomplete_prl_tail) =
            parse_list_level_grpprl(paragraph_property_bytes).map_err(|error| {
                Error::invalid(
                    ...
                    format!(
                        "LVL paragraph grpprl {paragraph_property_bytes:02x?} is invalid: {error}"
                    ),
                )
            })?;
    ...
    fn write(&self, bytes: &mut Vec<u8>) -> Result<()> {
        ...
        if self.paragraph_incomplete_prl_tail.len() > 10
            || self.number_incomplete_prl_tail.len() > 10
        {
            return Err(Error::invalid(
                0,
...
impl AssociatedStrings {
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let trailing_zero_words = u8::try_from(trailing.len() / 2).map_err(|_| {
            Error::invalid(input.offset as u64, "excessive SttbfAssoc zero padding")
        })?;
        let [
            unused0,
            template_path,
                ...
                match item {
                    ...
                    RecipientData::UniqueValue(value) => {
                        let byte_length = value
                            ...
                            .ok_or_else(|| {
                                Error::Limit("recipient unique value exceeds u16 bytes".into())
                            })?;
                        push_u16(&mut list, 4);
                ...
                match item {
                    ...
                    | FieldMapData::StandardFieldName(value) => {
                        let byte_length = value
                            ...
                            .and_then(|length| u16::try_from(length).ok())
                            .ok_or_else(|| {
                                Error::Limit("field-map name exceeds u16 bytes".into())
                            })?;
                        push_u16(
                            &mut list,
...
impl MailMergeStatus {
    ...
    fn to_u16(self) -> Result<u16> {
        Ok(u16::from(self.main_document_selected)
...
impl XmlSchemaReferences {
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let count = usize::try_from(count)
            .map_err(|_| Error::Limit("Hplxsdr count exceeds usize".into()))?;
...
impl FormatConsistencyProperties {
    ...
    fn to_u8(self) -> u8 {
        u8::from(self.character)
            | (u8::from(self.table) << 2)
            | (u8::from(self.line_separation) << 3)
    }
}
...
impl XmlSchemaStringTable {
    fn read(input: &mut SliceReader<'_>, name: &str) -> Result<Self> {
        ...
        let count = usize::try_from(count)
            .map_err(|_| Error::Limit(format!("{name} count exceeds usize")))?;
        if input.u16()? != 0 {
            return Err(Error::invalid(
                input.offset.saturating_sub(2) as u64,
...
impl SpellingState {
    ...
    fn validate(self) -> Result<()> {
        let error_is_valid = match self.kind {
            ...
            SpellingStateKind::MaybeDirty
            | SpellingStateKind::Foreign
            | SpellingStateKind::Clean => !self.error,
        };
        if !error_is_valid {
...
impl GrammarState {
    ...
    fn validate(self) -> Result<()> {
        let error_is_valid = match self.kind {
            GrammarStateKind::ErrorMin
            | GrammarStateKind::RepeatWord
            | GrammarStateKind::UnknownWord => self.error,
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        while input.offset < bytes.len() {
            ...
            let size = usize::try_from(input.u32()?)
                .map_err(|_| Error::Limit("Dofrh cb exceeds usize".into()))?;
            if size < 8 {
                return Err(Error::invalid(
                    start as u64,
    ...
    fn to_body(&self) -> Result<(u32, Vec<u8>)> {
        ...
        let kind = match self {
            ...
            Self::Frame(value) => {
                ...
                push_u32(
                    &mut body,
                    u32::from(value.linked)
                        | (u32::from(value.no_resize) << 1)
                        | (value.unused_flags << 2),
                );
...
impl GrammarCookieStore {
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let count = usize::try_from(input.u32()?)
            .map_err(|_| Error::Limit("RgCdb ccdb exceeds usize".into()))?;
    ...
    pub fn entry_offsets(&self) -> Result<Vec<u32>> {
        ...
        for cookie in &self.cookies {
            offsets.push(
                u32::try_from(offset)
                    .map_err(|_| Error::Limit("RgCdb entry offset exceeds u32".into()))?,
            );
            offset = offset
                .checked_add(4)
    ...
    pub fn validate_legacy_references(
        &self,
        table: &LegacyGrammarCheckerCookieTable,
    ) -> Result<()> {
        let offsets = self.entry_offsets()?.into_iter().collect::<BTreeSet<_>>();
        if table
            .cookies
...
impl GrammarCheckerCookieTable {
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        for _ in 0..count {
            ...
            if cookie.header
                && !header_languages.insert((cookie.language_sub, cookie.language_primary))
            {
                return Err(Error::invalid(
                    input.offset as u64 - 2,
        ...
        for bag in &self.property_bags {
            ...
            for property in &bag.properties {
                if usize::try_from(property.key_index).unwrap_or(usize::MAX) >= self.strings.len()
                    || usize::try_from(property.value_index).unwrap_or(usize::MAX)
                        >= self.strings.len()
                {
                    return Err(Error::invalid(0, "Property index is outside stringTable"));
                }
    ...
    fn write(&self, bytes: &mut Vec<u8>) -> Result<()> {
        ...
        push_u32(
            ...
            u32::try_from(body.len())
                .map_err(|_| Error::Limit("FactoidType size exceeds u32".into()))?,
        );
        bytes.extend_from_slice(&body);
        Ok(())
    }
...
impl RevisionMessageThreading {
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let message_attribute_count =
            read_threading_sttb_header(&mut input, 2, "SttbMessageAttrib")?;
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>> {
        ...
        push_u32(
            ...
            u32::try_from(self.ids.len())
                .map_err(|_| Error::Limit("PLRSID count exceeds u32".into()))?,
        );
            ...
            match record {
                ...
                CommandCustomizationRecord::CommandStrings(values) => {
                    ...
                    for value in values {
                        push_u16(
                            &mut bytes,
                            u16::try_from(value.value.len()).map_err(|_| {
                                Error::Limit("Tcg command string exceeds u16".into())
                            })?,
                        );
    ...
    fn write(&self, bytes: &mut Vec<u8>) -> Result<()> {
        ...
        for customization in &self.customizations {
            ...
            for delta in &customization.deltas {
                bytes.push(
                    (delta.operation & 0x03)
                        | (u8::from(delta.at_end) << 2)
                        | (delta.reserved << 3),
                );
...
impl DocumentPropertiesExtension {
    ...
    pub fn word2000_mut(&mut self) -> Option<&mut DocumentProperties2000> {
        match self {
            ...
            Self::Word2003WithTrailingByte { word2003, .. } => {
                Some(&mut word2003.word2002.word2000)
            }
...
impl DocumentProperties2000Flags {
    ...
    pub fn bits(self) -> Result<u32> {
        ...
        if self.web_options_initialized
            && (!self.target_screen_size.is_standard()
                || !(19..=480).contains(&self.pixels_per_inch))
        {
            return Err(Error::invalid(
                0,
                "Dop2000 initialized Web options are invalid",
        ...
        Ok(u32::from(self.language_detection_all_done)
...
impl DocumentProperties2000Flags2 {
    ...
    pub fn bits(self) -> Result<u16> {
        ...
        Ok(u16::from(self.suppress_page_boundaries)
...
impl DocumentProperties2002Flags {
    ...
    pub fn bits(self) -> Result<u16> {
        ...
        Ok(u16::from(self.do_not_embed_system_font)
...
impl DocumentProperties2003Flags {
    ...
    pub fn bits(self) -> Result<u32> {
        ...
        Ok(u32::from(self.treat_comment_lock_as_read_only)
...
impl DocumentMathProperties {
    fn read(input: &mut SliceReader<'_>) -> Result<Self> {
        ...
        let value = Self {
            ...
            binary_subtraction_break: MathBinarySubtractionBreak::from_u8(
                ((flags >> 2) & 3) as u8,
            )?,
...
impl DocumentStateFlags {
    ...
    pub fn bits(self) -> Result<u32> {
        ...
        Ok(u32::from(self.unused5_to_10)
...
impl EndnoteOptions {
    ...
    pub fn bits(self) -> Result<u16> {
        ...
        Ok(u16::from(self.placement.to_u8())
...
impl DocumentDrawingGrid {
    ...
    fn write(self, bytes: &mut Vec<u8>) -> Result<()> {
        ...
        bytes
            .push(self.horizontal_display_frequency.bits()? | (u8::from(self.follow_margins) << 7));
        Ok(())
    }
...
impl DocumentTypography {
    fn read(input: &mut SliceReader<'_>) -> Result<Self> {
        ...
        Ok(Self {
            ...
            justification: TypographyJustification::from_u8(((flags >> 1) & 3) as u8).ok_or_else(
                || Error::invalid(flags_offset, "DopTypography iJustification is invalid"),
            )?,
            kinsoku_level: KinsokuLevel::from_u8(((flags >> 3) & 3) as u8).ok_or_else(|| {
                Error::invalid(flags_offset, "DopTypography iLevelOfKinsoku is invalid")
            })?,
            ...
            custom_kinsoku_language: CustomKinsokuLanguage::from_u8(((flags >> 7) & 7) as u8)
                .ok_or_else(|| {
                    Error::invalid(flags_offset, "DopTypography iCustomKsu is invalid")
                })?,
    ...
    pub fn following_punctuation(&self) -> Result<&[u16]> {
        ...
        self.following_punctuation_slots
    ...
    pub fn leading_punctuation(&self) -> Result<&[u16]> {
        ...
        self.leading_punctuation_slots
...
impl LastListIndexes {
    ...
    pub fn matches_override_count(self, override_count: usize) -> bool {
        if override_count == 0 {
            ...
            usize::from(self.bullet) < override_count
                && usize::from(self.numbering) < override_count
        }
    }
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>> {
        ...
        for name in &self.names {
            push_u16(
                &mut bytes,
                u16::try_from(name.len())
                    .map_err(|_| Error::Limit("bookmark name exceeds u16".into()))?,
            );
            for character in name {
                push_u16(&mut bytes, *character);
            }
...
impl Sepx {
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>> {
        ...
        let length = i16::try_from(body.len())
            .map_err(|_| Error::Limit("Sepx grpprl exceeds i16".into()))?;
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        for style_index in 0..info.header.style_count {
            ...
            let definition = if length == 0 {
                ...
                let payload = input.bytes(length)?;
                Some(
                    StyleDefinition::from_bytes(payload, info.header.std_base_size).map_err(
                        |error| {
                            Error::invalid(
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>> {
        ...
        for style in &self.styles {
            let definition = style
                ...
                .transpose()?
                .unwrap_or_default();
            let length = i16::try_from(definition.len())
                .map_err(|_| Error::Limit("STD exceeds i16".into()))?;
            bytes.extend_from_slice(&length.to_le_bytes());
...
impl ChpxFkp {
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>> {
        ...
        if self.file_positions.len() != run_count.saturating_add(1)
            || !(1..=0x65).contains(&run_count)
        {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        for (raw_offset, paragraph_height_info) in bx_entries {
            ...
            let (length_encoding, body_start, body_length) = if cb == 0 {
                let cb_prime = *bytes.get(offset + 1).ok_or_else(|| {
                    Error::invalid(offset as u64, "truncated extended Papx length")
                })?;
                if cb_prime == 0 {
                    return Err(Error::invalid(
                        offset as u64,
...
impl Prl {
    fn read(input: &mut SliceReader<'_>) -> Result<Self> {
        ...
        let operand = match sprm.operand_size {
            ...
            SprmOperandSize::Byte if opcode == 0x3014 => SprmOperand::SectionHeaderFooterFlags(
                SectionHeaderFooterFlags::from_bits(input.u8()?),
            ),
            ...
            SprmOperandSize::Variable => {
                ...
                match opcode {
                    SPRM_P_CHG_TABS => {
                        SprmOperand::ParagraphChangeTabs(PChgTabsOperand::from_bytes(body)?)
                    }
                    0xc60d => {
                        SprmOperand::ParagraphChangeTabsPapx(PChgTabsPapxOperand::from_bytes(body)?)
                    }
                    ...
                    0xd632..=0xd634 | 0xd63e => {
                        SprmOperand::TableCellSpacing(Cssa::from_bytes(body)?)
                    }
                    0xd61a..=0xd61d => {
                        SprmOperand::TableBorderColors(ColorRef::array_from_bytes(body)?)
                    }
                    ...
                    0xd642 => {
                        SprmOperand::TableCellHideMark(CellHideMarkOperand::from_bytes(body)?)
                    }
                    ...
                    0xc645 => {
                        SprmOperand::ParagraphNumberRevisionMark(NumRmOperand::from_bytes(body)?)
                    }
                    ...
                    0xd202 => {
                        SprmOperand::OutlineListData(Box::new(OlstOperand::from_bytes(body)?))
                    }
    ...
    fn write(&self, bytes: &mut Vec<u8>) -> Result<()> {
        ...
        match (&self.operand, self.sprm.operand_size) {
            ...
            (SprmOperand::Byte(value), SprmOperandSize::Byte) if opcode != 0x3014 => {
                bytes.push(*value)
            }
            (SprmOperand::SectionHeaderFooterFlags(value), SprmOperandSize::Byte)
                if opcode == 0x3014 =>
            {
            ...
            (SprmOperand::Variable8(value), SprmOperandSize::Variable)
            ...
            (SprmOperand::CharacterFitText(value), SprmOperandSize::Variable)
                if opcode == 0xca76 =>
            {
                write_variable8(bytes, &value.to_bytes())?;
            }
            ...
            (SprmOperand::TableCellHideMark(value), SprmOperandSize::Variable)
                if opcode == 0xd642 =>
            {
                write_variable8(bytes, &value.to_bytes())?;
            }
            ...
            (SprmOperand::CharacterMajority(value), SprmOperandSize::Variable)
                if opcode == 0xca47 =>
            {
                write_variable8(bytes, &value.to_bytes()?)?;
            }
            ...
            (SprmOperand::AutoNumberedListData(value), SprmOperandSize::Variable)
                if opcode == 0xc63e =>
            {
                write_variable8(bytes, &value.to_bytes()?)?;
            }
            (SprmOperand::OutlineListData(value), SprmOperandSize::Variable)
                if opcode == 0xd202 =>
            {
                write_variable8(bytes, &value.to_bytes()?)?;
            }
...
impl PlcPcd {
    ...
    fn to_bytes(&self) -> Result<Vec<u8>> {
        ...
        let mut bytes =
            Vec::with_capacity(self.character_positions.len() * 4 + self.pieces.len() * 8);
        for position in &self.character_positions {
...
impl Pcd {
    pub fn text_piece(
        ...
        let bytes = word_document.get(start..end).ok_or_else(|| {
...
mod tests {
    ...
    fn fixture(version: FibVersion) -> Fib {
        let csw_new = match version {
            ...
            FibVersion::Word2000 | FibVersion::Word2002 | FibVersion::Word2003 => {
                FibRgCswNew::Word2000 {
    ...
    fn note_header_and_annotation_plcs_round_trip_static_records() {
        ...
        let office_art_bytes = [
            0x0f, 0x00, 0x00, 0xf0, 0, 0, 0, 0, 0, 0x0f, 0x00, 0x02, 0xf0, 12, 0, 0, 0, 0, 0, 0x10,
            0xf0, 4, 0, 0, 0, 0, 0, 0, 0,
        ];
    ...
    fn document_properties_97_round_trips_static_classification_space_lists_and_formats() {
        ...
        physical[492..494]
            .copy_from_slice(&u16::from(NumberingFormat::BULLET.code()).to_le_bytes());
    ...
    fn office_data_source_round_trips_all_static_property_variants() {
        let source = OfficeDataSource {
            properties: vec![
                OfficeDataSourceProperty::ConnectionString(
                    "Provider=Example".encode_utf16().collect(),
                ),
    ...
    fn range_protection_round_trips_prti_users_and_bookmarks() {
        ...
        invalid_cch[8] = 1;
        assert!(
            RangeProtection::from_bytes(&invalid_cch, &bytes.starts, &bytes.ends, &bytes.users)
                .is_err()
        );
    ...
    fn consistency_and_repair_bookmarks_round_trip_specialized_sttbs() {
        ...
        assert_eq!(
            FormatConsistencyBookmarks::from_bytes(&bytes.metadata, &bytes.starts, &bytes.ends,)
                .unwrap(),
            consistency
        );
        ...
        assert!(
            FormatConsistencyBookmarks::from_bytes(&invalid_fcct, &bytes.starts, &bytes.ends,)
                .is_err()
        );
    ...
    fn language_detection_state_table_round_trips_lad_spls_fields() {
        ...
        for invalid_mask in [0x0020_u16, 0x0040, 0x0080] {
            let mut invalid = bytes.clone();
            let encoded = u16::from_le_bytes([invalid[state_offset], invalid[state_offset + 1]])
                | invalid_mask;
