    ...
    pub fn from_bytes_with_limits(bytes: &[u8], limits: Limits) -> Result<Self> {
        ...
        while cursor < bytes.len() {
            ...
            let payload = bytes.get(cursor..end).ok_or_else(|| {
                Error::invalid(record_offset as u64, "truncated VBA dir record payload")
            })?;
            cursor = end;
...
fn take_bytes<'a>(
    ...
    let value = payload.get(*cursor..end).ok_or_else(|| {
        Error::invalid(offset as u64 + *cursor as u64, format!("truncated {name}"))
    })?;
    *cursor = end;
    Ok(value)
}
...
fn write_length_prefixed(bytes: &mut Vec<u8>, value: &[u8]) -> Result<()> {
    let len = u32::try_from(value.len())
        .map_err(|_| Error::Limit("VBA string length exceeds u32".into()))?;
