        ...
        while cursor < bytes.len() {
            ...
            let kind = if line.is_empty() {
                ...
                match section {
                    ...
                    Section::HostExtenders if line.contains(&b'=') => {
                        ProjectRecordKind::HostExtender {
                            value: line.to_vec(),
                        }
                    }
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        for record in &self.records {
            match &record.kind {
                ...
                ProjectRecordKind::HostExtenderHeader => {
                    bytes.extend_from_slice(b"[Host Extender Info]")
                }
                ProjectRecordKind::HostExtender { value }
                | ProjectRecordKind::Workspace { value } => bytes.extend_from_slice(value),
...
impl ProjectWmStream {
    ...
    pub fn from_bytes_with_limits(bytes: &[u8], limits: Limits) -> Result<Self> {
        ...
        loop {
            ...
            loop {
                let pair = bytes.get(cursor..cursor.saturating_add(2)).ok_or_else(|| {
                    Error::invalid(cursor as u64, "unterminated PROJECTwm Unicode name")
                })?;
                cursor += 2;
...
impl ProjectLkStream {
    ...
    pub fn from_bytes_with_limits(bytes: &[u8], limits: Limits) -> Result<Self> {
        ...
        for _ in 0..count {
            let class_id: [u8; 16] =
                take_bytes(bytes, &mut cursor, 16, "truncated PROJECTlk ClassID")?
                    .try_into()
            ...
            let license_required =
                take_u32(bytes, &mut cursor, "truncated PROJECTlk LicenseRequired")?;
