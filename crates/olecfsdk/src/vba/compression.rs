    ...
    pub fn from_bytes_with_limits(bytes: &[u8], limits: Limits) -> Result<Self> {
        ...
        while !remaining.is_empty() {
            ...
            let data = if header.is_compressed() {
                let (sequences, output_len) = parse_token_sequences(
                    data_bytes,
                    container_offset + 2,
                    decompressed_len,
                    limits,
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>> {
        ...
        for chunk in &self.chunks {
            ...
            if chunk.header.is_compressed()
                != matches!(chunk.data, CompressedChunkData::Compressed(_))
            {
                return Err(Error::invalid(
                    offset as u64,
        ...
        match self {
            ...
            Self::Compressed(sequences) => {
                ...
                for (sequence_index, sequence) in sequences.iter().enumerate() {
                    ...
                    if (!is_final_flag_only && sequence.tokens.is_empty())
                        || sequence.tokens.len() > 8
                    {
                        return Err(Error::invalid(
