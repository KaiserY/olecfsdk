...
impl<R: Read + Seek> CompoundFileReader<R> {
    ...
    pub fn from_reader_with_buffer_size(
        ...
        let mut source =
            SeekSectorSource::new(reader, original_len, &header, limits.max_file_size)?;
...
impl<R: Read + Write + Seek> CfbStreamMut<'_, R> {
    ...
    pub fn set_len(&mut self, new_len: u64) -> io::Result<()> {
        ...
        self.write_directory_stream_location(self.stream_id)
            .map_err(as_io_error)?;
        self.len = new_len;
    ...
    fn migrate_mini_to_regular(&mut self, new_len: u64) -> io::Result<()> {
        ...
        for _ in 0..desired {
            ...
            if let Some(&previous) = regular_chain.last() {
                self.set_fat_entry(previous, FatEntry::Sector(id))
                    .map_err(as_io_error)?;
            }
        ...
        write_regular_at(self.source, &regular_chain, new_len, 0, &preserved)
            .map_err(as_io_error)?;
        self.directory
            .entry_mut(self.stream_id)
            .expect("validated directory entry remains present")
        ...
        for id in old_mini_chain {
            self.set_mini_fat_entry(id, MiniFatEntry::Free)
                .map_err(as_io_error)?;
        }
    ...
    fn migrate_regular_to_mini(&mut self, new_len: u64) -> io::Result<()> {
        ...
        read_regular_at(self.source, &old_chain, self.len, 0, &mut preserved)
            .map_err(as_io_error)?;
        ...
        for _ in 0..desired {
            ...
            if let Some(&previous) = mini_chain.last() {
                self.set_mini_fat_entry(previous, MiniFatEntry::MiniSector(id))
                    .map_err(as_io_error)?;
            }
            mini_chain.push(id);
        }
        ...
        self.directory
            ...
            .start_sector = start;
        for id in old_chain {
            self.set_fat_entry(id, FatEntry::Free)
                .map_err(as_io_error)?;
        }
    ...
    fn resize_regular(&mut self, new_len: u64) -> io::Result<()> {
        ...
        if desired > chain.len() {
            ...
            self.set_fat_entry(last, FatEntry::EndOfChain)
                .map_err(as_io_error)?;
            for id in released {
                self.set_fat_entry(id, FatEntry::Free)
                    .map_err(as_io_error)?;
    ...
    fn resize_mini(&mut self, new_len: u64) -> io::Result<()> {
        ...
        if desired > mini_chain.len() {
            ...
            if let Some(&last) = mini_chain.last() {
                self.set_mini_fat_entry(last, MiniFatEntry::EndOfChain)
                    .map_err(as_io_error)?;
            }
            for id in released {
                self.set_mini_fat_entry(id, MiniFatEntry::Free)
    ...
    fn allocate_mini_sector(
        ...
        let id = if let Some(id) = reusable {
            ...
            let id =
                MiniSectorId::new(u32::try_from(root_count).map_err(|_| {
                    Error::Limit("allocated mini-sector ID does not fit u32".into())
                })?)?;
            let new_root_len = root_len
                .checked_add(MINI_SECTOR_LEN as u64)
                .ok_or_else(|| Error::Limit("root mini stream length overflow".into()))?;
            let needed_regular =
                usize::try_from(new_root_len.div_ceil(self.source.sector_len() as u64))
...
mod tests {
    ...
    fn shrinking_streams_zeroes_unused_regular_and_mini_sector_tails() {
        ...
        let regular_index = compound
            ...
            .unwrap();
        let StreamChain::Regular(regular_chain) = compound.stream_chain(regular_index).unwrap()
        else {
    ...
    fn file_backed_storage_copy_uses_bounded_stream_transfers() {
        let (bytes, large, small) = compound_bytes();
        let mut compound = CompoundFileReader::from_reader_with_buffer_size(
            Cursor::new(bytes),
            Limits::default(),
            4096,
        )
