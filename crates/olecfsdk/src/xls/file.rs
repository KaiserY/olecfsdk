...
pub struct XlsSparseCellIndex<'a> {
    ...
    formula_groups:
        BTreeMap<(u16, u16, XlsFormulaGroupTokenKind), Vec<XlsFormulaDefinitionRef<'a>>>,
    cell_count: usize,
}
...
impl RevisionLogStream {
    ...
    pub fn relationships_compatible(&self) -> Result<XlsRevisionStreamView<'_>> {
        ...
        let user_exclusion = take_prefix(|data| matches!(data, BiffRecordData::UsrExcl(_)))
            .and_then(|record| match &record.data {
...
impl<'a> XlsStoragesAndStreams<'a> {
    ...
    pub fn by_role(
        &self,
        role: XlsFileEntryRole,
    ) -> impl Iterator<Item = XlsFileEntryRef<'a>> + '_ {
        self.entries
            .iter()
            .copied()
    ...
    fn resolve_object_persistence_with_policy<'view>(
        ...
        if flags.contains(ObjPictureFlags::DDE)
            && formula_starts_with_table
        ...
        if flags.contains(ObjPictureFlags::CONTROL_STREAM) {
            ...
            let size = usize::try_from(size)
                .map_err(|_| Error::Limit("Ctls object size exceeds usize".into()))?;
            let end = start
                .checked_add(size)
...
impl<'a> XlsSheetRef<'a> {
    ...
    pub fn custom_sheet_views_compatible(
        ...
        while cursor < self.records.len() {
            ...
            let begin = match &begin_record.data {
                ...
                BiffRecordData::UserSViewBeginChart(value) => {
                    XlsCustomSheetViewBeginRef::Chart(value)
                }
        ...
        for record in self.direct_records() {
            ...
            if let Some(previous_record) = previous {
                let locator = match &previous_record.data {
                    BiffRecordData::Formula(value)
                    | BiffRecordData::Formula4Compatibility(value) => Some(value.cell),
                    _ => None,
                };
                if let Some(locator) = locator {
                    let definition = match &record.data {
                        BiffRecordData::SharedFormula(value) => {
                            Some(XlsFormulaDefinitionRef::Shared(value))
                        }
    ...
    pub fn resolve_formula_compatible(
        self,
        formula: &'a FormulaRecord,
        ...
        let definition = match group_reference {
            ...
            Some((row, column, token_kind)) => {
                ...
                match first {
                    ...
                    None if preserve_compatibility => {
                        XlsFormulaDefinitionRef::UnresolvedExp { row, column }
                    }
                    None => {
                        return Err(Error::invalid(
                            u64::from(source_record.offset),
    ...
    formula
        ...
        .and_then(|token| match token.data {
            FormulaTokenData::Exp { row, column } => {
                Some((row, column, XlsFormulaGroupTokenKind::Exp))
            }
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            let value = match &record.data {
                BiffRecordData::Obj(value) | BiffRecordData::ObjCompatibility { value, .. } => {
                    value
                }
                _ => continue,
            };
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            let direct = match &record.data {
                BiffRecordData::Formula(value) => {
                    Some((value.cell, XlsCellValueRef::Formula(value)))
                }
                BiffRecordData::Formula4Compatibility(value) => {
                ...
                BiffRecordData::BoolErr(value) => {
                    Some((value.cell, XlsCellValueRef::BoolErr(value)))
                }
                BiffRecordData::Label(value) => Some((value.cell, XlsCellValueRef::Label(value))),
                BiffRecordData::LabelSst(value) => {
                    Some((value.cell, XlsCellValueRef::LabelSst(value)))
                }
...
impl<'a> XlsWorkbookView<'a> {
        ...
        let name = supporting_link
            ...
            .ok_or_else(|| {
                Error::invalid(
                    0,
                    format!(
                        "PtgNameX references missing one-based ExternName {one_based_name_index}"
                    ),
                )
            })?;
    ...
    pub fn defined_name_scope(&self, one_based_index: u32) -> Result<Option<XlsSheetRef<'a>>> {
        ...
        self.sheet_at_position(usize::from(name.sheet_index - 1))
            .map(Some)
            .ok_or_else(|| {
                Error::invalid(
                    0,
    ...
    pub fn resolve_formula_token_defined_name(
        ...
        match token {
            FormulaTokenData::Name { name_index } => {
                self.resolve_defined_name(*name_index).map(Some)
            }
            _ => Ok(None),
        }
    }
    ...
    pub fn resolve_shared_string(&self, index: u32) -> Result<&'a SstString> {
        self.shared_string(index)?.ok_or_else(|| {
            Error::invalid(0, format!("LabelSst references missing SST string {index}"))
        })
    }
    ...
    pub fn resolve_cell_shared_string(
        &self,
        cell: XlsCellRef<'a>,
    ) -> Result<Option<&'a SstString>> {
        cell.label_sst()
    ...
    pub fn xfs(&self) -> impl Iterator<Item = &'a XfRecord> {
        self.globals_records()
            .iter()
            .filter_map(|record| match &record.data {
                BiffRecordData::Xf(value) | BiffRecordData::XfCompatibility { value, .. } => {
                    Some(value)
                }
                _ => None,
            })
    ...
    pub fn resolve_font(&self, index: u16) -> Result<&'a FontRecord> {
        self.font(index).ok_or_else(|| {
            Error::invalid(0, format!("XF references invalid or missing Font {index}"))
        })
    }
    ...
    pub fn resolve_number_format(&self, index: u16) -> Result<XlsNumberFormatRef<'a>> {
        ...
        self.custom_number_format(index)
            .map(XlsNumberFormatRef::Custom)
            .ok_or_else(|| {
                Error::invalid(0, format!("XF references missing custom Format {index}"))
            })
    }

    pub fn resolve_number_format_compatible(&self, index: u16) -> XlsNumberFormatRef<'a> {
        self.resolve_number_format(index)
...
fn name_value_eq_ascii(value: &NameValue, expected: &[u8]) -> bool {
    match value {
        ...
        NameValue::User(XlStringCharacters::Compressed(bytes)) => {
            bytes.eq_ignore_ascii_case(expected)
        }
        NameValue::User(XlStringCharacters::Unicode(words)) => {
            words.len() == expected.len()
                && words.iter().zip(expected).all(|(word, byte)| {
                    u8::try_from(*word).is_ok_and(|word| word.eq_ignore_ascii_case(byte))
                })
        }
    }
}
...
impl XlsWorkbookStream {
    ...
    fn relationships_with_policy(
        &self,
        preserve_compatibility: bool,
    ) -> Result<XlsWorkbookView<'_>> {
        if self
            .sheet_ids
            .iter()
            .copied()
        ...
        for sheet_view in &all_custom_sheet_views {
            let resolved_sheet = workbook_sheet_identifiers.and_then(|identifiers| {
                let mut positions = identifiers
                    ...
                    .enumerate()
                    .filter(|(_, identifier)| {
                        u32::from(**identifier) == sheet_view.sheet_identifier()
                    })
                    ...
                    .contains(super::UserBViewFlags::INVALID_SHEET_ID)
                {
                    let active_position = if let Some(identifiers) = workbook_sheet_identifiers {
                        let matches = identifiers
                            ...
                            .enumerate()
                            .filter(|(_, identifier)| {
                                **identifier == custom_view.value.active_sheet_id
                            })
                            .map(|(position, _)| position)
                            .collect::<Vec<_>>();
                        if matches.len() == 1 {
...
fn remap_self_referencing_sheet_range(
    ...
    reference.first_sheet_index = u16::try_from(mapped[0])
...
fn remap_workbook_sheet_positions(
    ...
    let self_referencing_sup_books = records
        .iter()
        .filter_map(|record| match &record.data {
            BiffRecordData::SupBook(value) => {
                Some(matches!(value.link, SupBookLink::SelfReference))
            }
            _ => None,
        })
        .collect::<Vec<_>>();
...
impl XlsFile {
    ...
    fn reorder_sheets_with_policy(
        ...
        let mut workbook_indices = rebuilt
            ...
            .map(|(index, _)| index);
        let workbook_index = workbook_indices.next().ok_or_else(|| {
            Error::invalid(0, format!("missing {} BIFF stream", workbook_name.path()))
        })?;
    ...
    fn edit_cell_with_policy<T>(
        ...
        let target = {
            ...
            let cell = index.cell(row, column)?.ok_or_else(|| {
                Error::invalid(0, format!("sheet has no cell at ({row}, {column})"))
            })?;
            let record = workbook
                .tree
                .stream
        ...
        let cell = match (
            ...
            (BiffRecordData::Blank(value), XlsCellMutationTarget::Record(_)) => {
                XlsCellMut::Blank(value)
            }
            ...
            (BiffRecordData::Label(value), XlsCellMutationTarget::Record(_)) => {
                XlsCellMut::Label(value)
            }
            ...
            .filter(|entry| entry.kind != EntryKind::Root)
        {
            let role = if is_root_child(entry) {
                ...
                match parse_fixed_hex_name(&entry.name, b"", 4) {
                    Some(cache_id) if entry.kind == EntryKind::Stream => {
        ...
        for log in stream.revision_logs.iter().copied() {
            ...
            for source in revisions {
                ...
                let local_sheet_identifier = match &source {
                    XlsRevisionRecordRef::DefinedName { value, .. }
                        if value.local_sheet_id != u16::MAX =>
                    {
        ...
        if let Some(storage) = pivot_cache_storage {
            for entry in compound_file.entries().iter().filter(|entry| {
                entry.kind == EntryKind::Stream
                    && entry.path.parent() == Some(storage.path.as_path())
            }) {
                ...
                match PivotCacheStream::from_bytes_with_limits(&entry.data, options.limits) {
                    Ok(stream) => {
                        if stream.properties().is_some_and(|properties| {
                            properties.has_zero_length_refresh_user_compatibility()
                        }) {
                    ...
                    Ok(value) => {
                            ...
                            && let Err(error) = value.validate()
                        {
                            diagnostics.push(ParseDiagnostic::warning(
                                ...
                                format!(
                                    "preserved a typed nonconforming User Names Stream: {error}"
                                ),
                            ));
                        }
            ...
            .filter(|entry| {
                entry.kind == EntryKind::Stream
                    && entry.path.parent().is_some_and(|parent| {
                        compound.entry(parent).is_some_and(|storage| {
                            storage.name.eq_ignore_ascii_case(PIVOT_CACHE_STORAGE_NAME)
                        })
                    })
...
mod tests {
    ...
    use crate::{
        ...
        xls::{
            ...
            ColInfoReserved, DevModeFields, DevModeWPublic, ExtSstRecord, FontAttributes,
            FontRecord, FormatRecord, FormulaCachedResult, FormulaRecord,
            FormulaSpecialCachedResult, FormulaToken, FormulaTokenStream, FormulaTokens, FrtFlags,
            FrtHeaderOld, IsstInf, NumberRecord, ShortXlUnicodeString, SupBookLink, XfRecord,
            XlStringCharacters,
        },
    };
