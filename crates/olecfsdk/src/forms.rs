...
impl VariousPropertiesBitfield {
    ...
    pub fn with_ime_mode(self, value: FmImeMode) -> Self {
        Self::from_bits_retain(
            (self.bits() & !Self::IME_MODE.bits()) | (u32::from(value.raw()) << 15),
        )
    }
...
impl PageProperties {
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>> {
        ...
        let size = u16::try_from(bytes.len() - 4)
            .map_err(|_| Error::Limit("cbPage exceeds u16".into()))?;
...
impl TabStripControl {
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let extra_data_block =
            TabStripExtraDataBlock::read(&mut cursor, property_mask, &data_block)?;
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>> {
        ...
        self.extra_data_block
            .write(&mut bytes, self.property_mask, &self.data_block)?;
        let size = u16::try_from(bytes.len() - 4)
            .map_err(|_| Error::Limit("cbTabStrip exceeds u16".into()))?;
        bytes[2..4].copy_from_slice(&size.to_le_bytes());
        write_picture(
            &mut bytes,
            self.property_mask
...
impl FormObjectStream {
    ...
    pub fn to_bytes(&self, form: &FormControl) -> Result<Vec<u8>> {
        ...
        for ((expected_site_index, site), control) in expected_sites.into_iter().zip(&self.controls)
        {
            ...
            if control.site_index != expected_site_index
                || control.class_index != expected_class_index
            {
                return Err(Error::invalid(
                    0,
...
fn embedded_parent_class_id(site: &OleSiteConcreteControl) -> Result<Guid> {
    match site_class_index(site) {
        ...
        SiteClassIndex::Cached(CachedControlClass::Frame) => {
            Ok(ParentControlStorage::FRAME_CLASS_ID)
        }
...
fn validate_parent_children(
    ...
    for (site_index, site) in expected {
        let child = actual.next().ok_or_else(|| {
            Error::invalid(0, "ParentControlStorage is missing an embedded parent")
        })?;
        let storage_name = site_storage_name(site)?;
        let expected_class_id = embedded_parent_class_id(site)?;
...
impl TabStripExtraDataBlock {
    fn read(
        ...
        Ok(Self {
            ...
            tip_strings: read_sized_array_strings(
                cursor,
...
impl FormExtraDataBlock {
    fn read(
        ...
        Ok(Self {
            ...
            scroll_position: read_fm_position(
                cursor,
...
impl FormSiteData {
    fn from_prefix(bytes: &[u8], class_table_omitted: bool) -> Result<(Self, usize)> {
        ...
        let mut class_table =
            Vec::with_capacity(usize::from(count_of_site_class_info.unwrap_or(0)));
        ...
        for _ in 0..count_of_sites {
            let (value, size) =
                OleSiteConcreteControl::from_prefix(&bytes[cursor.position..cursor.end])?;
            cursor.position += size;
            sites.push(value);
        }
...
impl OleSiteConcreteControl {
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>> {
        ...
        self.extra_data_block
            .write(&mut bytes, self.property_mask, &self.data_block)?;
        let size = u16::try_from(bytes.len() - 4)
            .map_err(|_| Error::Limit("cbSite exceeds u16".into()))?;
        bytes[2..4].copy_from_slice(&size.to_le_bytes());
        Ok(bytes)
    }
}
...
impl SiteDataBlock {
    fn read(cursor: &mut SliceCursor<'_>, mask: SitePropertyMask) -> Result<Self> {
        ...
        value.control_tip_text =
            read_count_descriptor(cursor, mask.contains(Mask::CONTROL_TIP_TEXT))?;
...
impl CommandButtonControl {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        Ok(Self {
            ...
            major_version,
            property_mask,
            take_focus_on_click: !property_mask
...
fn write_cached_form_control(
    ...
    match (value, class) {
        ...
        (FormControlPersistence::SpinButton(value), CachedControlClass::SpinButton) => {
            value.to_bytes()
        }
        ...
        (FormControlPersistence::ScrollBar(value), CachedControlClass::ScrollBar) => {
            value.to_bytes()
        }
        _ => Err(Error::invalid(
            0,
...
impl MorphDataControl {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let extra_data_block =
            MorphDataExtraDataBlock::read(&mut cursor, property_mask, &data_block)?;
        if cursor.position != property_end {
            return Err(Error::invalid(
                cursor.position as u64,
...
impl TextProps {
    fn from_prefix(bytes: &[u8]) -> Result<(Self, usize)> {
        ...
        let extra_data_block =
            TextPropsExtraDataBlock::read(&mut cursor, property_mask, &data_block)?;
        if cursor.position != end {
            return Err(Error::invalid(
                cursor.position as u64,
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>> {
        ...
        self.extra_data_block
            .write(&mut bytes, self.property_mask, &self.data_block)?;
        let size = u16::try_from(bytes.len() - 4)
            .map_err(|_| Error::Limit("cbTextProps exceeds u16".into()))?;
        bytes[2..4].copy_from_slice(&size.to_le_bytes());
        Ok(bytes)
    }
}
        ...
        (Some(descriptor), Some(values)) => {
            ...
            for value in values {
                ...
                encoded.extend_from_slice(
                    &(value.character_count | if value.compressed { 0x8000_0000 } else { 0 })
                        .to_le_bytes(),
                );
...
fn validate_morph_cached_class(value: &MorphDataControl, class: CachedControlClass) -> Result<()> {
    ...
    let effective_class = match (class, display_style) {
        ...
        (CachedControlClass::MorphDataLegacy, FmDisplayStyle::CheckBox) => {
            CachedControlClass::CheckBox
        }
...
fn validate_control_various(
    ...
    let allowed = match class {
        ...
        CachedControlClass::CommandButton => {
            common | Bits::LOCKED | Bits::WORD_WRAP | Bits::AUTO_SIZE
        }
...
mod tests {
    ...
    fn common_property_types_are_static_and_byte_exact() {
        ...
        <FmSpecialEffect as FormScalar<u32>>::write_le(&FmSpecialEffect::Bump, &mut encoded)
            .unwrap();
    ...
    fn remaining_cached_controls_round_trip_minimal_static_shapes() {
        let image = [
            0x00, 0x02, 0x0c, 0x00, 0x00, 0x02, 0x00, 0x00, 0xe8, 0x03, 0x00, 0x00, 0xf4, 0x01,
            0x00, 0x00,
        ];
        assert_eq!(
        ...
        let label = [
            0x00, 0x02, 0x0c, 0x00, 0x20, 0x00, 0x00, 0x00, 0xe8, 0x03, 0x00, 0x00, 0xf4, 0x01,
        ...
        let spin_button = [
            0x00, 0x02, 0x0c, 0x00, 0x08, 0x00, 0x00, 0x00, 0xe8, 0x03, 0x00, 0x00, 0xf4, 0x01,
            0x00, 0x00,
        ];
        assert_eq!(
    ...
    fn form_control_round_trips_empty_page_container() {
        let bytes = [
            0x00, 0x04, 0x1c, 0x00, 0x40, 0x0c, 0x00, 0x08, 0x04, 0x80, 0x00, 0x00, 0x00, 0x7d,
            0x00, 0x00, 0x07, 0x07, 0x00, 0x00, 0x2b, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    fn multi_page_x_round_trips_page_and_id_properties() {
        let bytes = [
            0x00, 0x02, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x04, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x02, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x0c, 0x00,
            0x06, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x04, 0x00,
    ...
    fn morph_data_preserves_legacy_low_word_string_length() {
        let bytes = [
            0x00, 0x02, 0x28, 0x00, 0x17, 0x01, 0x00, 0x86, 0x00, 0x00, 0x00, 0x00, 0x1b, 0x48,
            0x80, 0x2c, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00,
            0xc0, 0xc0, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x00, 0xb2, 0x0e, 0x00, 0x00, 0x4b, 0x02,
            0x00, 0x00, 0x00, 0x02, 0x1c, 0x00, 0x37, 0x00, 0x00, 0x00, 0x07, 0x00, 0x04, 0x80,
            0x01, 0x00, 0x00, 0x00, 0xb4, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x56, 0x65,
            0x72, 0x64, 0x61, 0x6e, 0x61, 0x00,
        ];
