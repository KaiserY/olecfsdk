    ...
    pub fn from_bytes(bytes: &[u8], code_page: u16) -> Result<Self> {
        ...
        for _ in 0..count {
            ...
            let len = usize::try_from(reader.read_u32()?).map_err(|_| {
                Error::Limit("OLEPS dictionary name length does not fit usize".into())
            })?;
        ...
        for entry in &self.entries {
            ...
            match &entry.name {
                DictionaryName::Mbcs(bytes) => {
                    writer.write_u32(
                        u32::try_from(bytes.len()).map_err(|_| {
                            Error::Limit("OLEPS dictionary name exceeds u32".into())
                        })?,
                    )?;
                ...
                DictionaryName::Unicode {
                    ...
                    writer.write_u32(u32::try_from(code_units.len()).map_err(|_| {
                        Error::Limit("OLEPS dictionary Unicode name exceeds u32".into())
                    })?)?;
        ...
        let value = match property_type {
            ...
            PropertyType::CF => {
                let size = usize::try_from(reader.read_u32()?).map_err(|_| {
                    Error::Limit("OLEPS ClipboardData size does not fit usize".into())
                })?;
                if size < 4 {
                    return Err(Error::invalid(
                        reader.position()?,
            ...
            PropertyType::VECTOR_BSTR
            | PropertyType::VECTOR_LPSTR
            | PropertyType::VECTOR_LPWSTR => {
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>> {
        ...
        match self {
            ...
            Self::ClipboardData {
                ...
                writer
                    .write_u32(u32::try_from(size).map_err(|_| {
                        Error::Limit("OLEPS ClipboardData size exceeds u32".into())
                    })?)?;
    ...
    fn write_to<W: Write + std::io::Seek>(&self, writer: &mut Writer<W>) -> Result<()> {
        ...
        match self {
            ...
            Self::CodePageStrings(values) => {
                for value in values {
                    writer.write_u32(u32::try_from(value.bytes.len()).map_err(|_| {
                        Error::Limit("OLEPS string vector element exceeds u32".into())
                    })?)?;
            ...
            Self::UnicodeStrings(values) => {
                for value in values {
                    writer.write_u32(u32::try_from(value.code_units.len()).map_err(|_| {
                        Error::Limit("OLEPS Unicode vector element exceeds u32".into())
                    })?)?;
...
fn read_variant_scalar<R: std::io::Read + std::io::Seek>(
    ...
    let value = match property_type {
        ...
        PropertyType::LPSTR | PropertyType::BSTR => {
            let len = usize::try_from(reader.read_u32()?).map_err(|_| {
                Error::Limit("OLEPS variant string length does not fit usize".into())
            })?;
        ...
        PropertyType::LPWSTR => {
            let count = usize::try_from(reader.read_u32()?).map_err(|_| {
                Error::Limit("OLEPS variant Unicode length does not fit usize".into())
            })?;
            reader.ensure_allocation(count, 2)?;
            let mut code_units = Vec::with_capacity(count);
            for _ in 0..count {
        ...
        PropertyType::CF => {
            let size = usize::try_from(reader.read_u32()?).map_err(|_| {
                Error::Limit("OLEPS variant clipboard size does not fit usize".into())
            })?;
            if size < 4 {
                return Err(Error::invalid(
                    offset,
...
fn parse_string_vector(
    ...
    if property_type == PropertyType::VECTOR_LPWSTR {
        let mut values = Vec::with_capacity(count);
        for _ in 0..count {
            let code_unit_count = usize::try_from(reader.read_u32()?).map_err(|_| {
                Error::Limit("OLEPS Unicode vector length does not fit usize".into())
            })?;
        ...
        for _ in 0..count {
            let len = usize::try_from(reader.read_u32()?).map_err(|_| {
                Error::Limit("OLEPS string vector length does not fit usize".into())
            })?;
...
impl PropertySetStream {
    ...
    pub fn from_bytes_with_limits(bytes: &[u8], limits: Limits) -> Result<Self> {
        ...
        for (index, (format_identifier, offset)) in descriptors.iter().enumerate() {
            ...
            let property_set = parse_property_set(
                bytes.get(start..bound).ok_or_else(|| {
                    Error::invalid(*offset as u64, "property set is out of bounds")
                })?,
                *format_identifier,
                limits,
            )?;
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>> {
        ...
        for section in &sections {
            offsets.push(
                u32::try_from(next).map_err(|_| Error::Limit("OLEPS offset exceeds u32".into()))?,
            );
            next = next
...
impl PropertySet {
    ...
    fn to_bytes(&self) -> Result<Vec<u8>> {
        ...
        for property in &self.properties {
            offsets.push(
                u32::try_from(next)
                    .map_err(|_| Error::Limit("property offset exceeds u32".into()))?,
            );
            next = next
                .checked_add(property.raw.len())
                .ok_or_else(|| Error::Limit("property set size overflow".into()))?;
        }
        let size = u32::try_from(next)
            .map_err(|_| Error::Limit("property set size exceeds u32".into()))?;
