...
impl MsoEnvelope {
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>> {
        ...
        self.sent_representing_name
            .write(&mut writer, self.version)?;
        self.internet_account_stamp
            .write(&mut writer, self.version)?;
        self.internet_account_name
            .write(&mut writer, self.version)?;
...
impl EnvelopeRecipientPropertyValue {
    ...
    fn write<W: Write + Seek>(&self, writer: &mut Writer<W>) -> Result<()> {
        match self {
            ...
            Self::Unicode(value) => {
                write_u16_utf16_bytes(writer, value, "recipient Unicode property")
            }
...
fn add_context(error: Error, name: &str) -> Error {
    match error {
        Error::InvalidData { offset, message } => {
            Error::invalid(offset, format!("{name}: {message}"))
        }
        other => other,
    }
}
